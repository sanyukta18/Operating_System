...
int main()
{
	...
	//key_t key = ftok("lavda", 65);
	 int shmid = shmget(IPC_PRIVATE,40000000,IPC_CREAT|0600);
	...
	if (flag == 0)
	{
		...
		while ((*s).job_created < ((*s).NJ))
		{
			if ((*s).q.is_full())
			{
				cout<<"Queue full.Producer Waiting...\n";
				continue;
			}
	...
	if (flag == 0)
	{
		...
		while ((*s).job_completed < ((*s).NJ))
		{
			if ((*s).q.empty())
				continue;
			bool flag1 = 0;
			int mat1[5*5]; int mat2[5*5]; int res[5*5];
			for(int i=0;i<5;i++)
			{
				for(int j=0;j<5;j++)
				{
				...
				else
				{
					...
					for(int i = 0; i < 5; ++i)
	        			for(int j = 0; j < 5; ++j)
	           				 for(int k = 0; k < 5; ++k)
				            {
				                ...
				                res[i*5+j] += mat1[i*5+k] * mat2[k*5+j];
				            }
				     ...
				     for(int i = rr1;i<=rr2;i++)
					{
						for(int j = rc1;j<=rc2;j++)
						{
							...
							r[i*10+j] = r[i*10+j]+res[(i-rr1)*5+(j-rc1)];
						}
					}
